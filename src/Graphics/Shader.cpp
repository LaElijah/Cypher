	...
	: Shader<OpenGLShader>(infoData)
{
	...
	unsigned int attrIndex = 1;
		...
		std::string attrName = matches.str(3);
                std::regex_search(attrType, sizeMatch, sizeRegex);
                std::regex_search(attrType, typeMatch, typeRegex);
		...
		m_Attributes.push_back(Graphics::OpenGLVertexAttribute{
		        ...
		        GetPrimitiveBaseType(GLSLPrimitives[typeMatch[0]]),    // Base Type
		        GLSLPrimitives[typeMatch[0]],                          // Component Type
		        offset,                                                // offset
		        GL_FALSE		                               // normalize
		    });

		offset += GetPrimitiveSize(GLSLPrimitives[typeMatch[0]]);
		formatKey += GLSLPrimitives[typeMatch[0]] * (16 + attrIndex);
		++attrIndex;
	    }
        } // END LOOP	
