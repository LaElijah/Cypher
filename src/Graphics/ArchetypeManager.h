...
namespace Graphics
{
    ...
    public:
        ...
        bool add(Graphics::Entity entity, T component)
        {
            ...
            if (
                archetypeID != entityID && ARCHETYPES.count(archetypeID) == 0)
                ARCHETYPES.emplace(
                    archetypeID,
            ...
            if (!entityInfo.contains(std::type_index(typeid(T)).name()))
            {
                if (!entityInfo.isZero()) // is this a new entity?
                {
                    ...
                    // Cleanup
                    std::vector<std::byte> *byteVector = new std::vector<std::byte>(); // storage for component copy
        ...
        bool remove(Graphics::Entity entity)
        {
            ...
            if (
                archetypeInfo.getID() != entityInfo.getID() && ARCHETYPES.count(archetypeID) == 0)
                ARCHETYPES.emplace(
                    archetypeID,
                    std::make_shared<Graphics::Archetype>(archetypeInfo));
