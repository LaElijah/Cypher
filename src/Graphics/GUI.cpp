...
{
    return (mousePos.x >= contentRegionMin.x && mousePos.x <= contentRegionMax.x && mousePos.y >= contentRegionMin.y && mousePos.y <= contentRegionMax.y);
}
...
void Graphics::GUI::handleClick()
{
         ...
         it != ImGui::GetCurrentContext()->Windows.begin();)
    {
        ...
        if (isWithinWindow(mousePos, windowPos, window->Size))
        {
            ...
            if (isWithinContentRegion(mousePos, contentRegionMin, contentRegionMax) && (loggedComponents.find(window->Name) != loggedComponents.end()))
            {
